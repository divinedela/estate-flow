        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            <div>
              ...
              <span className={`ml-2 px-3 py-1 text-sm font-medium rounded-full ${statusColors[lead.status || 'new'] || statusColors.new}`}>
                {lead.status || 'new'}
              </span>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-500">Priority:</span>
              <span className={`ml-2 px-3 py-1 text-sm font-medium rounded-full ${priorityColors[lead.priority || 'medium'] || priorityColors.medium}`}>
            ...
            {lead.interest_type && (
              <div>
                ...
                <span className="ml-2 text-sm font-semibold text-gray-900 capitalize">{lead.interest_type}</span>
              </div>
            )}
          </div>
                ...
                <div className="p-6">
                  ...
                  <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900 capitalize">{lead.lead_source || 'N/A'}</dd>
                    </div>
                    {campaign && (
                      <div>
                    ...
                    {assignedUser && (
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-gray-900">{assignedUser.full_name || assignedUser.email}</dd>
                      </div>
                    )}
                    {lead.preferred_property_type && (
                      <div>
                        <dt className="text-sm font-medium text-gray-500">Preferred Property Type</dt>
                        <dd className="mt-1 text-sm text-gray-900 capitalize">{lead.preferred_property_type}</dd>
                      </div>
                    )}
                    {lead.preferred_location && (
                      <div className="sm:col-span-2">
                        <dt className="text-sm font-medium text-gray-500">Preferred Location</dt>
                        <dd className="mt-1 text-sm text-gray-900">{lead.preferred_location}</dd>
                      </div>
                    )}
                    {lead.expected_timeline && (
                      <div>
                        <dt className="text-sm font-medium text-gray-500">Expected Timeline</dt>
                        <dd className="mt-1 text-sm text-gray-900">{lead.expected_timeline}</dd>
                      </div>
                    )}
                    {lead.next_follow_up_date && (
                      <div>
                        <dt className="text-sm font-medium text-gray-500">Next Follow-up</dt>
                        <dd className="mt-1 text-sm text-gray-900">{formatDateTime(lead.next_follow_up_date)}</dd>
                      </div>
                    )}
                  </dl>
                </div>
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-2">
                    <div>
                      ...
                      <dd className="mt-1 text-lg font-semibold text-gray-900">{formatCurrency(lead.budget_min ? Number(lead.budget_min) : null)}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Maximum</dt>
                      <dd className="mt-1 text-lg font-semibold text-gray-900">{formatCurrency(lead.budget_max ? Number(lead.budget_max) : null)}</dd>
                    </div>
                  </div>
                </div>
              </Card>
                  ...
                  <dl className="space-y-2">
                    ...
                    {lead.converted_at && (
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-gray-900">{formatDateTime(lead.converted_at)}</dd>
                      </div>
                    )}
                  </dl>
                </div>
