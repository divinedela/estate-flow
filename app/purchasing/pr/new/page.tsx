  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validItems = prItems.filter(
        (item) => item.description && item.quantity && parseFloat(item.quantity) > 0
      )
      ...
      const itemsData = validItems.map((item) => ({
        ...
        estimated_total_price: item.estimated_total_price ? parseFloat(item.estimated_total_price) : null,
