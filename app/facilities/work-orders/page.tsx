...
export default function WorkOrdersPage() {
  ...
  async function fetchData() {
    ...
    const [workOrdersRes, requestsRes] = await Promise.all([
      supabase
        ...
        .select('*, maintenance_request:maintenance_requests(title, request_number)')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const laborCost = formData.labor_cost ? parseFloat(formData.labor_cost) : null
    const materialCost = formData.material_cost ? parseFloat(formData.material_cost) : null
    ...
    if (editingOrder) {
      ...
      if (formData.status === 'in_progress' && editingOrder.status !== 'in_progress' && !editingOrder.started_at) {
        updateData.started_at = new Date().toISOString()
      }
      if (formData.status === 'completed' && editingOrder.status !== 'completed') {
  ...
  async function updateStatus(id: string, status: string, currentOrder: WorkOrder) {
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <Card>
              <div className="text-center p-4">
                ...
                <p className="text-2xl font-bold text-indigo-600">${stats.totalCost.toLocaleString()}</p>
          ...
          <Card>
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="mt-4 text-lg font-medium text-gray-900">No work orders found</h3>
                <p className="mt-2 text-gray-500">Create a new work order to get started.</p>
                  ...
                  {filteredOrders.map((order) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell className="text-sm text-gray-600">
                        {(order.maintenance_request as { request_number: string } | undefined)?.request_number || '-'}
                      </TableCell>
                      ...
                      <TableCell>
                        <select
                          ...
                          className={`text-xs font-medium rounded-full px-2 py-1 border-0 ${statusColors[order.status]}`}
                        >
                          <option value="scheduled">Scheduled</option>
                          <option value="in_progress">In Progress</option>
                          <option value="completed">Completed</option>
        ...
        >
          {viewingOrder && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-semibold text-gray-900">{viewingOrder.title}</h3>
                  <p className="text-gray-500">{viewingOrder.work_order_number}</p>
                </div>
                <span className={`px-3 py-1 text-sm font-medium rounded-full ${statusColors[viewingOrder.status]}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">
                    {(viewingOrder.maintenance_request as { request_number: string } | undefined)?.request_number || 'N/A'}
                  </p>
                </div>
                <div>
