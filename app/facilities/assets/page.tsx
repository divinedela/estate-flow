...
export default function AssetsPage() {
  ...
  async function fetchData() {
    ...
    const [assetsRes, facilitiesRes, unitsRes] = await Promise.all([
      supabase
        .from('assets')
        .select('*, facility:facilities(name), unit:facility_units(unit_number)')
        .order('created_at', { ascending: false }),
      supabase.from('facilities').select('id, name, facility_code').order('name'),
      supabase.from('facility_units').select('id, unit_number, facility_id').order('unit_number'),
    ])
    ...
    <RoleGuard allowedRoles={['super_admin', 'facility_manager']}>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            ...
            <Card>
              <div className="text-center p-4">
                <p className="text-2xl font-bold text-orange-600">{stats.warrantyExpiring}</p>
            ...
            <Card className="border-l-4 border-l-orange-500 bg-orange-50">
              <div className="flex items-center p-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-orange-600">{stats.warrantyExpiring} asset(s) have warranties expiring within 30 days</p>
                </div>
              </div>
            </Card>
          )}
              ...
              <Table>
                ...
                <TableBody>
                  {filteredAssets.map((asset) => {
                    const warrantyExpired = asset.warranty_expiry_date && new Date(asset.warranty_expiry_date) < today
                    const warrantyExpiringSoon = asset.warranty_expiry_date && !warrantyExpired && (() => {
                    ...
                    return (
                      <TableRow key={asset.id}>
                        ...
                        <TableCell className="text-sm text-gray-600">
                          {(asset.facility as { name: string } | undefined)?.name || asset.location_description || '-'}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          {asset.warranty_expiry_date ? (
                            <span className={`text-sm ${warrantyExpired ? 'text-red-600' : warrantyExpiringSoon ? 'text-orange-600' : 'text-gray-600'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormSelect
                ...
                onChange={(e) => setFormData({ ...formData, facility_id: e.target.value, unit_id: '' })}
              >
          ...
          {viewingAsset && (
            <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-gray-900">{viewingAsset.name}</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{viewingAsset.asset_type?.replace('_', ' ') || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Status</p>
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[viewingAsset.status]}`}>
                    {viewingAsset.status.replace('_', ' ')}
                  </span>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Manufacturer</p>
                  <p className="font-medium">{viewingAsset.manufacturer || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Model</p>
                  <p className="font-medium">{viewingAsset.model_number || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Serial Number</p>
                  <p className="font-medium">{viewingAsset.serial_number || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Location</p>
                  <p className="font-medium">
                    {(viewingAsset.facility as { name: string } | undefined)?.name || viewingAsset.location_description || 'N/A'}
                  </p>
                </div>
              </div>

              <div className="bg-gray-50 p-4 rounded-lg">
                <h4 className="font-medium text-gray-900 mb-3">Purchase & Warranty</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {viewingAsset.installation_date 
                        ? new Date(viewingAsset.installation_date).toLocaleDateString() 
                        : 'N/A'}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Warranty Expiry</p>
                    <p className={`font-medium ${viewingAsset.warranty_expiry_date && new Date(viewingAsset.warranty_expiry_date) < today ? 'text-red-600' : ''}`}>
                      {viewingAsset.warranty_expiry_date 
                        ? new Date(viewingAsset.warranty_expiry_date).toLocaleDateString() 
                        : 'N/A'}
                    </p>
                  </div>
                </div>
