...
export default function PreventiveMaintenancePage() {
  ...
  const [viewingSchedule, setViewingSchedule] = useState<PreventiveSchedule | null>(null)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const scheduleData = {
      ...
      frequency_value: formData.frequency_value ? parseInt(formData.frequency_value) : null,
  ...
  async function markCompleted(schedule: PreventiveSchedule) {
    ...
    switch (schedule.frequency_type) {
      case 'daily':
        ...
        break
      case 'weekly':
        nextDue.setDate(nextDue.getDate() + 7 * (schedule.frequency_value || 1))
        break
      case 'monthly':
  ...
  const filteredSchedules = schedules.filter(schedule => {
    const matchesSearch = schedule.schedule_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (schedule.asset as { name: string } | undefined)?.name?.toLowerCase().includes(searchQuery.toLowerCase())
    
    if (statusFilter === 'active') return matchesSearch && schedule.is_active
    if (statusFilter === 'inactive') return matchesSearch && !schedule.is_active
    if (statusFilter === 'overdue') {
      return matchesSearch && schedule.next_due_date && new Date(schedule.next_due_date) < today
    }
  ...
  const stats = {
    ...
    overdue: schedules.filter(s => s.next_due_date && new Date(s.next_due_date) < today && s.is_active).length,
    ...
    completedThisMonth: schedules.filter(s => {
      ...
      return lastPerformed.getMonth() === today.getMonth() && lastPerformed.getFullYear() === today.getFullYear()
    }).length,
  }
            ...
            <Card className="border-l-4 border-l-red-500 bg-red-50">
              <div className="flex items-center p-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-600">{stats.overdue} schedule(s) are overdue and require immediate attention</p>
                </div>
              </div>
            </Card>
          )}
          ...
          <Card>
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="mt-2 text-gray-500">Create a preventive maintenance schedule to get started.</p>
                      ...
                      <TableRow key={schedule.id} className={!schedule.is_active ? 'opacity-50' : ''}>
                        ...
                        <TableCell>
                          <span className="px-2 py-1 text-xs font-medium rounded bg-indigo-100 text-indigo-800">
                            ...
                            {schedule.frequency_value && schedule.frequency_type === 'custom' && ` (${schedule.frequency_value} days)`}
                          </span>
                        </TableCell>
                        <TableCell className="text-sm text-gray-500">
                          {schedule.last_performed_date 
                            ? new Date(schedule.last_performed_date).toLocaleDateString() 
                            : 'Never'}
                        </TableCell>
                        <TableCell>
                          {schedule.next_due_date ? (
                            <div className="flex items-center">
                              {isOverdue && <ExclamationTriangleIcon className="h-4 w-4 text-red-500 mr-1" />}
                              {isDueSoon && !isOverdue && <ClockIcon className="h-4 w-4 text-yellow-500 mr-1" />}
                              <span className={`text-sm ${isOverdue ? 'text-red-600 font-medium' : isDueSoon ? 'text-yellow-600' : 'text-gray-600'}`}>
        ...
        >
          {viewingSchedule && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-semibold text-gray-900">{viewingSchedule.schedule_name}</h3>
                  <p className="text-gray-500">{(viewingSchedule.asset as { name: string } | undefined)?.name || 'No asset'}</p>
                </div>
                <span className={`px-3 py-1 text-sm font-medium rounded-full ${
                  viewingSchedule.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                }`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">
                    ...
                    {viewingSchedule.frequency_value && viewingSchedule.frequency_type === 'custom' && 
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {viewingSchedule.last_performed_date 
                      ? new Date(viewingSchedule.last_performed_date).toLocaleDateString() 
                      : 'Never'}
                  </p>
                </div>
                <div>
                  ...
                  <p className={`font-medium ${
                    viewingSchedule.next_due_date && new Date(viewingSchedule.next_due_date) < today 
                      ? 'text-red-600' 
                      : ''
                  }`}>
