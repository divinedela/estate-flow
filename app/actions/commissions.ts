...
export async function getCommission(commissionId: string) {
  try {
    ...
    return { success: false, error: error.message || 'Failed to fetch commission' }
  }
}
 ...
 * Update commission
 */
export async function updateCommission(commissionId: string, data: Partial<CommissionFormData>) {
  try {
    const supabase = await createClient()
    ...
    if (data.dealDescription !== undefined) updateData.deal_description = data.dealDescription
    if (data.transactionDate) updateData.transaction_date = data.transactionDate
    if (data.expectedPaymentDate !== undefined) updateData.expected_payment_date = data.expectedPaymentDate
    ...
    if (data.saleAmount || data.commissionRate || data.splitPercentage) {
      ...
      const saleAmount = data.saleAmount ?? existing?.sale_amount ?? 0
      const commissionRate = data.commissionRate ?? existing?.commission_rate ?? 0
      const splitPercentage = data.splitPercentage ?? existing?.split_percentage ?? 100
...
export async function approveCommission(commissionId: string) {
  try {
    ...
    return { success: false, error: error.message || 'Failed to approve commission' }
  }
}
...
export async function rejectCommission(commissionId: string, reason?: string) {
  try {
    ...
    return { success: false, error: error.message || 'Failed to reject commission' }
  }
}
...
export async function markCommissionPaid(
  ...
  try {
    ...
    return { success: false, error: error.message || 'Failed to mark commission as paid' }
  }
}
...
export async function deleteCommission(commissionId: string) {
  try {
    ...
    return { success: false, error: error.message || 'Failed to delete commission' }
  }
}
...
export async function getCommissionStats(filters?: {
  ...
  try {
    ...
    const stats = {
      ...
      totalAmount: commissions?.reduce((sum, c) => sum + Number(c.final_commission || 0), 0) || 0,
      pendingAmount: commissions?.filter(c => c.status === 'pending').reduce((sum, c) => sum + Number(c.final_commission || 0), 0) || 0,
      paidAmount: commissions?.filter(c => c.status === 'paid').reduce((sum, c) => sum + Number(c.final_commission || 0), 0) || 0,
    }
