...
export async function createUser(input: CreateUserInput) {
  try {
    ...
    const { data: userRoles } = await supabase
      ...
      .eq("user_id", (currentAppUser as any).id);
    ...
    const { data: appUserData, error: appUserError } = await (
      adminClient.from("app_users") as any
    )
      .insert({
...
export async function updateUser(input: UpdateUserInput) {
  try {
    ...
    if (!hasPermission) {
      return {
        error:
          "Unauthorized: Only super admins or HR managers can update users",
      };
    }
    ...
    // Update app_user
    const { error: updateError } = await (adminClient.from("app_users") as any)
      .update({
        full_name: input.full_name,
        phone: input.phone || null,
        organization_id: input.organization_id,
    ...
    if (input.role_id) {
      ...
      await adminClient.from("user_roles").delete().eq("user_id", input.id);
      ...
      if (roleError) {
        ...
        return {
          error: "User updated but failed to update role: " + roleError.message,
        };
      }
    }
