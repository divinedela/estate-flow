...
export default function UsersPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const usersWithRoles = (usersData || []).map((user: any) => {
        const userRole = (userRolesData || []).find((ur: any) => ur.user_id === user.id)
        return {
          ...user,
  ...
  function openCreateModal() {
    ...
    setFormData({
      ...
      role_id: roles.find(r => r.name === 'executive')?.id || roles[0]?.id || '',
      is_active: true,
    })
    setIsModalOpen(true)
  }
  ...
  async function toggleUserStatus(user: AppUser) {
    try {
      ...
      setMessage({ 
        type: 'success', 
        text: `User ${!user.is_active ? 'activated' : 'deactivated'} successfully` 
      })
      fetchData()
    } catch (error: any) {
  ...
  async function deleteUser(user: AppUser) {
    if (!confirm(`Are you sure you want to delete ${user.full_name || user.email}? This will remove them from the system but not delete their login credentials.`)) {
      return
    }
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesRole = !roleFilter || 
      user.user_roles?.some(ur => ur.role?.id === roleFilter)
  ...
  const stats = {
    ...
    noRole: users.filter(u => !u.user_roles || u.user_roles.length === 0).length,
  }
            ...
            <div className={`rounded-lg p-4 ${
              ...
              <div className="flex items-start">
                {message.type === 'success' ? (
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-green-600 mr-2 flex-shrink-0">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                ) : (
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-red-600 mr-2 flex-shrink-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <Card>
              <div className="text-center">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 text-gray-400 mx-auto">
            ...
            <Card>
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-yellow-600 mt-2">{stats.noRole}</p>
                        ...
                        <TableCell>
                          {user.user_roles && user.user_roles.length > 0 ? (
                            <span className="px-2 py-1 text-xs font-medium rounded-full bg-indigo-100 text-indigo-800">
                              {user.user_roles[0]?.role?.name?.replace('_', ' ') || 'Unknown'}
                            </span>
                          ) : (
            ...
            {!editingUser && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> The user will receive their login credentials. They can use the email and password you set to log in.
                </p>
              </div>
            )}
