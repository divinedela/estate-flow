...
async function getCampaignStats() {
  ...
  if (!profile?.organization_id) return { total: 0, active: 0, completed: 0, draft: 0 }
    ...
    <RoleGuard allowedRoles={['super_admin', 'marketing_officer']}>
        <div className="space-y-6">
          ...
          <div className="flex space-x-2 overflow-x-auto pb-2">
            <Button variant="secondary" className="whitespace-nowrap">All ({stats.total})</Button>
            <Button variant="secondary" className="whitespace-nowrap">Active ({stats.active})</Button>
            <Button variant="secondary" className="whitespace-nowrap">Completed ({stats.completed})</Button>
            <Button variant="secondary" className="whitespace-nowrap">Draft ({stats.draft})</Button>
          </div>
                ...
                {campaigns.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} className="text-center text-gray-500 py-8">
                      No campaigns found.{' '}
                      <Link href="/marketing/campaigns/new" className="text-indigo-600 hover:text-indigo-500">
                  ...
                  campaigns.map((campaign: any) => (
                    <TableRow key={campaign.id}>
                      ...
                      <TableCell>
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${typeColors[campaign.type] || 'bg-gray-100 text-gray-800'}`}>
                          {campaign.type || 'N/A'}
                        </span>
                      </TableCell>
                      ...
                      <TableCell>
                        <span className="font-medium">{campaign.leads_generated || 0}</span>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          <Link
