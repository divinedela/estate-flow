...
async function getLeadStats() {
  ...
  // Stage counts
  const newLeads = ((leads || []) as any[]).filter(
    (l: any) => l.status === "new"
  ).length;
  const contacted = ((leads || []) as any[]).filter(
    (l: any) => l.status === "contacted"
  ).length;
  const qualified = ((leads || []) as any[]).filter(
    (l: any) => l.status === "qualified"
  ).length;
  const converted = ((leads || []) as any[]).filter(
    (l: any) => l.status === "converted"
  ).length;

  // Temperature counts (using temperature field, fallback to status for backwards compatibility)
  const hot = ((leads || []) as any[]).filter(
    (l: any) =>
      l.temperature === "hot" || (!l.temperature && l.status === "hot")
  ).length;
  const warm = ((leads || []) as any[]).filter(
    (l: any) =>
      l.temperature === "warm" || (!l.temperature && l.status === "warm")
  ).length;
  const cold = ((leads || []) as any[]).filter(
    (l: any) =>
      l.temperature === "cold" || (!l.temperature && l.status === "cold")
  ).length;
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex space-x-2 overflow-x-auto pb-2">
            <Button
              variant="secondary"
              className="whitespace-nowrap bg-red-50 hover:bg-red-100 border-red-200"
            >
              🔥 Hot ({stats.hot})
            </Button>
            <Button
              variant="secondary"
              className="whitespace-nowrap bg-orange-50 hover:bg-orange-100 border-orange-200"
            >
              ☀️ Warm ({stats.warm})
            </Button>
            <Button
                ...
                leads.map((lead: any) => {
                  ...
                  const temp =
                    ...
                    (["hot", "warm", "cold"].includes(lead.status)
                      ...
                      : "warm");
                  const tempConfig =
                    temperatureConfig[temp] || temperatureConfig.warm;
                  ...
                  return (
                    <TableRow key={lead.id}>
                      ...
                      <TableCell className="capitalize">
                        {(lead.lead_source || "direct").replace(/_/g, " ")}
                      </TableCell>
                      <TableCell>
                        <span
                          className={`px-2 py-1 text-xs font-medium rounded-full ${
                            stageColors[lead.status || "new"] || stageColors.new
                          }`}
                        >
                          {lead.status || "new"}
                        </span>
                      ...
                      <TableCell>
                        <span
                          className={`px-2.5 py-1 text-xs font-medium rounded-full ${tempConfig.bg}`}
                        >
