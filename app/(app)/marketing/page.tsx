...
async function getMarketingStats() {
  ...
  const hotLeads = leads.filter(
    (l) => l.temperature === "hot" || (!l.temperature && l.status === "hot")
  ).length;
  const warmLeads = leads.filter(
    (l) => l.temperature === "warm" || (!l.temperature && l.status === "warm")
  ).length;
  const coldLeads = leads.filter(
    (l) => l.temperature === "cold" || (!l.temperature && l.status === "cold")
  ).length;
  // Stage-based counts
            ...
            {stats.leadsBySource.length === 0 ? (
              ...
              <div className="space-y-4">
                {stats.leadsBySource.map((item, index) => {
                  ...
                  const displaySource =
                    sourceLabels[item.source] || item.source.replace(/_/g, " ");
                ...
                <div className="flex h-2 rounded-full overflow-hidden bg-gray-200">
                  {stats.hotLeads > 0 && (
                    <div
                      ...
                      style={{
                        width: `${(stats.hotLeads / stats.totalLeads) * 100}%`,
                      }}
                    />
                  )}
                  {stats.warmLeads > 0 && (
                    <div
                      ...
                      style={{
                        width: `${(stats.warmLeads / stats.totalLeads) * 100}%`,
                      }}
                    />
                  )}
                  {stats.coldLeads > 0 && (
                    <div
                      ...
                      style={{
                        width: `${(stats.coldLeads / stats.totalLeads) * 100}%`,
                      }}
                    />
                  )}
