...
export default function NewCommissionPage() {
  ...
  useEffect(() => {
    ...
    const commissionAmount = (formData.saleAmount * formData.commissionRate) / 100
  ...
  const loadAgents = async () => {
    ...
    if (result.success) {
      setAgents(result.data.filter((a: any) => a.employment_status === 'active'))
    }
    setLoading(false)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Record Commission</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                ...
                {agents.map((agent: any) => (
                  <option key={agent.id} value={agent.id}>
                    {agent.first_name} {agent.last_name} ({agent.agent_code}) - {agent.commission_rate}%
                  </option>
                ))}
              </select>
              {agents.length === 0 && (
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <label htmlFor="dealDescription" className="block text-sm font-medium text-gray-700 mb-2">
                Deal Description *
              </label>
              <input
                type="text"
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, saleAmount: parseFloat(e.target.value) || 0 })}
                className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
            ...
            <div>
              <label htmlFor="splitPercentage" className="block text-sm font-medium text-gray-700 mb-2">
                Split Percentage (%) *
              </label>
              <input
                type="number"
                ...
                onChange={(e) => setFormData({ ...formData, splitPercentage: parseFloat(e.target.value) || 100 })}
                className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
              <p className="mt-1 text-xs text-gray-500">
                100% if agent receives full commission, less if split with others
              </p>
            </div>

            <div>
              <label htmlFor="expectedPaymentDate" className="block text-sm font-medium text-gray-700 mb-2">
          ...
          <div className="mt-6 p-4 bg-indigo-50 rounded-lg border border-indigo-200">
            <h4 className="text-sm font-semibold text-indigo-900 mb-3">Commission Breakdown</h4>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-700">Sale Amount:</span>
                <span className="font-medium text-gray-900">{formatCurrency(formData.saleAmount)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-700">Commission Rate:</span>
                <span className="font-medium text-gray-900">{formData.commissionRate}%</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-700">Commission Amount:</span>
                <span className="font-medium text-gray-900">{formatCurrency(calculatedCommission.commissionAmount)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-700">Split Percentage:</span>
                <span className="font-medium text-gray-900">{formData.splitPercentage}%</span>
              </div>
              <div className="h-px bg-indigo-200 my-2"></div>
              <div className="flex justify-between text-base">
                <span className="font-semibold text-indigo-900">Final Commission:</span>
                <span className="font-bold text-indigo-600 text-lg">{formatCurrency(calculatedCommission.finalCommission)}</span>
              </div>
            </div>
          </div>
        </Card>
