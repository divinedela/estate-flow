...
async function getInventoryStats() {
  ...
  const totalInbound = ((todayTxns || []) as any[])
    .filter(
      (t: any) =>
        t.transaction_type === "inbound" || t.transaction_type === "return"
    )
    .reduce((sum, t: any) => sum + Number(t.quantity), 0);
  const totalOutbound = ((todayTxns || []) as any[])
    .filter((t: any) => t.transaction_type === "outbound")
    .reduce((sum, t: any) => sum + Number(t.quantity), 0);
  ...
  if (reorderRules && reorderRules.length > 0) {
    for (const rule of reorderRules as any[]) {
      const { data: stockLevel } = await supabase
        ...
        .eq("location_id", (rule as any).location_id)
        .single();

      if (
        stockLevel &&
        Number((stockLevel as any).quantity) <=
          Number((rule as any).min_quantity)
      ) {
        lowStockItems++;
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p
                  className={`${
                    stats.lowStockItems > 0 ? "text-red-100" : "text-gray-100"
                  } text-sm font-medium`}
                >
                  Low Stock Alerts
                </p>
              ...
              <div
                className={`p-3 ${
                  stats.lowStockItems > 0 ? "bg-red-400/30" : "bg-gray-400/30"
                } rounded-lg`}
              >
            ...
            <div className="flex items-center">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-red-600">
                  {stats.lowStockItems} item(s) have reached their reorder
                  point. Review and reorder to avoid stockouts.
                </p>
              </div>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span
                            className={`px-2 py-1 text-xs font-medium rounded-full ${
                              transactionTypeColors[txn.transaction_type] ||
            ...
            {stats.lowStockAlerts.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Set up reorder rules to get alerts
                </p>
              </div>
            ) : (
