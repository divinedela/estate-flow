...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    if (formData.item_id) {
      ...
      if (item?.unit_price && !formData.unit_cost) {
        setFormData(prev => ({ ...prev, unit_cost: item.unit_price?.toString() || '' }))
      }
    } else {
      setCurrentStock([])
  ...
  function validateTransaction(): string | null {
    ...
    if (formData.transaction_type === 'outbound') {
      ...
      if (qty > available) {
        return `Insufficient stock. Available: ${available} ${selectedItemInfo?.unit_of_measure || 'units'}`
      }
    }
    ...
    if (formData.transaction_type === 'transfer') {
      ...
      if (qty > available) {
        return `Insufficient stock at source. Available: ${available} ${selectedItemInfo?.unit_of_measure || 'units'}`
      }
    }
    
    if (formData.transaction_type === 'inbound' && !formData.to_location_id && !formData.location_id) {
      ...
      if (formData.transaction_type === 'transfer' && formData.to_location_id) {
        const { error: transferError } = await supabase
          ...
          .insert({
            ...
            transaction_type: 'inbound',
            notes: `Transfer from ${locations.find(l => l.id === formData.from_location_id)?.name || 'unknown'}. ${formData.notes || ''}`.trim(),
          } as any)
  ...
  const filteredTransactions = transactions.filter(txn => {
    ...
    const matchesLocation = !locationFilter || txn.location?.name === locationFilter
  ...
  const stats = {
    ...
    inbound: transactions.filter(t => t.transaction_type === 'inbound').length,
    outbound: transactions.filter(t => t.transaction_type === 'outbound').length,
    transfers: transactions.filter(t => t.transaction_type === 'transfer').length,
    todayValue: todayTransactions.reduce((sum, t) => sum + (t.total_cost || 0), 0),
  }
  ...
  const uniqueLocations = [...new Set(transactions.map(t => t.location?.name).filter(Boolean))]
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <Card>
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-green-600 mt-1">{stats.inbound}</p>
            ...
            <Card>
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-red-600 mt-1">{stats.outbound}</p>
            ...
            <Card>
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-blue-600 mt-1">{stats.transfers}</p>
                        ...
                        <TableRow key={txn.id}>
                          <TableCell>
                            <div>
                              ...
                              <p className="text-xs text-gray-500">{new Date(txn.created_at).toLocaleTimeString()}</p>
                            </div>
                          </TableCell>
                          <TableCell>
                            <div>
                              <p className="font-medium text-gray-900">{txn.item?.name || 'N/A'}</p>
                              <p className="text-xs text-gray-500">{txn.item?.item_code}</p>
                            </div>
                          </TableCell>
                          <TableCell>
                            <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${typeColors[txn.transaction_type] || 'bg-gray-100 text-gray-800'}`}>
                              {typeIcons[txn.transaction_type]}
                              <span className="ml-1 capitalize">{txn.transaction_type}</span>
                            </span>
                          </TableCell>
                          <TableCell>
                            <span className={`font-medium ${
                              ...
                              {txn.transaction_type === 'outbound' ? '-' : txn.transaction_type === 'inbound' ? '+' : ''}
                          ...
                          <TableCell>
                            <div className="flex items-center">
                              ...
                              <span className="text-sm">{txn.location?.name || 'N/A'}</span>
                            </div>
                          </TableCell>
                          <TableCell>{formatCurrency(txn.unit_cost)}</TableCell>
                          <TableCell className="font-medium">{formatCurrency(txn.total_cost)}</TableCell>
                          <TableCell>
                            {txn.reference_type ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="border-b pb-4">
              ...
              <FormSelect
                ...
                onChange={(e) => setFormData({ ...formData, item_id: e.target.value, unit_cost: '' })}
                options={[
              ...
              {selectedItemInfo && currentStock.length > 0 && (
                <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                  <p className="text-sm font-medium text-blue-800 mb-2">Current Stock Levels:</p>
                  <div className="grid grid-cols-2 gap-2">
                    {currentStock.map(stock => {
                      const loc = locations.find(l => l.id === stock.location_id)
                      return (
                        <div key={stock.location_id} className="text-sm text-blue-700">
                          {loc?.name}: <span className="font-medium">{stock.quantity} {selectedItemInfo.unit_of_measure}</span>
                        </div>
                      )
                    })}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormSelect
                  ...
                  options={[
                    ...
                    { value: 'adjustment', label: '🔧 Adjustment (Stock Correction)' },
                    { value: 'return', label: '↩️ Return (Returned Stock)' },
                  ]}
                />
                <FormInput
                  label={`Quantity (${selectedItemInfo?.unit_of_measure || 'units'})`}
                  type="number"
                  min="0.001"
                  step="0.001"
            ...
            <div className="border-b pb-4">
              ...
              {formData.transaction_type === 'transfer' ? (
                  ...
                  <div>
                    <FormSelect
                      ...
                      options={[
                        ...
                        ...locations.map(loc => {
                          ...
                          return { 
                            value: loc.id, 
                            label: `${loc.name}${loc.country ? ` (${loc.country})` : ''} - Stock: ${stock}` 
                          }
                        })
                      ]}
                    />
                    {formData.from_location_id && (
                      <p className="text-xs text-gray-500 mt-1">
                        Available: {getStockAtLocation(formData.from_location_id)} {selectedItemInfo?.unit_of_measure || 'units'}
                      </p>
                    )}
                  </div>
                  <FormSelect
              ...
              ) : formData.transaction_type === 'inbound' || formData.transaction_type === 'return' ? (
                <FormSelect
                ...
                <div>
                  ...
                  />
                  {formData.from_location_id && formData.transaction_type === 'outbound' && (
                    <p className="text-xs text-gray-500 mt-1">
                      Available: {getStockAtLocation(formData.from_location_id)} {selectedItemInfo?.unit_of_measure || 'units'}
                    </p>
                  )}
                ...
                <div>
                  ...
                  <div className="px-4 py-2 bg-gray-100 rounded-lg text-gray-900 font-medium">
                    {formData.unit_cost && formData.quantity 
                      ? formatCurrency(parseFloat(formData.unit_cost) * parseFloat(formData.quantity))
                      : '-'
                    }
                  </div>
                </div>
            ...
            {/* Reference & Project */}
            <div className="border-b pb-4">
              <h4 className="text-sm font-medium text-gray-700 mb-3">Reference (Optional)</h4>
              <div className="grid grid-cols-2 gap-4">
                <FormSelect
              ...
              <div className="mt-4">
                <FormSelect
                  ...
                  options={[
                    ...
                    ...projects.map(p => ({ value: p.id, label: `${p.name} (${p.project_code})` }))
                  ]}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{new Date(viewingTransaction.transaction_date).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Type</p>
                  <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${typeColors[viewingTransaction.transaction_type]}`}>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{viewingTransaction.item?.item_code}</p>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <p className="text-sm text-gray-500">Quantity</p>
                  <p className="font-medium">{viewingTransaction.quantity} {viewingTransaction.item?.unit_of_measure}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Unit Cost</p>
                  <p className="font-medium">{formatCurrency(viewingTransaction.unit_cost)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Total Cost</p>
                  <p className="font-medium">{formatCurrency(viewingTransaction.total_cost)}</p>
                </div>
              </div>
              <div>
                <p className="text-sm text-gray-500">Location</p>
                <p className="font-medium">{viewingTransaction.location?.name || 'N/A'}</p>
              </div>
              ...
              {viewingTransaction.reference_type && (
                <div>
                  ...
                  <p className="font-medium">{viewingTransaction.reference_type}: {viewingTransaction.reference_id}</p>
                </div>
              )}
              ...
              <div className="pt-4 border-t">
                <p className="text-xs text-gray-400">
                  Created: {new Date(viewingTransaction.created_at).toLocaleString()}
                </p>
              </div>
            </div>
          )}
