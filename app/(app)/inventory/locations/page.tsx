...
export default function LocationsPage() {
  ...
  async function fetchLocations() {
    ...
    const locationsWithCount = await Promise.all(
      ((locationsData || []) as any[]).map(async (loc: any) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingLocation) {
      const { error } = await (supabase.from("stock_locations") as any)
        .update(locationData)
      ...
      const { error } = await (supabase.from("stock_locations") as any).insert(
        locationData
      );
  ...
  async function handleDelete(location: StockLocation) {
    if (location.item_count && location.item_count > 0) {
      alert(
        "Cannot delete location with stock. Please transfer all items first."
      );
      return;
    }

    if (confirm(`Are you sure you want to delete "${location.name}"?`)) {
      await supabase.from("stock_locations").delete().eq("id", location.id);
  ...
  async function toggleStatus(location: StockLocation) {
    await (supabase.from("stock_locations") as any)
  ...
  const locationCountries = [
    ...new Set(locations.map((l) => l.country_code).filter(Boolean)),
  ];
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Manage warehouses, stores, and inventory locations across
                countries
              </p>
            </div>
          </div>
                      ...
                      <TableRow key={location.id}>
                        ...
                        <TableCell>
                          <span className="font-medium">
                            {getCurrencySymbol(location.currency)}
                          </span>
