...
export default function AttendancePage() {
  ...
  async function fetchAttendance() {
    ...
    const filteredData = (rawData || [])
      .filter(a => a.employee?.organization_id === profile.organization_id)
      .map(a => ({
        id: a.id,
  ...
  async function handleDelete(record: AttendanceLog) {
    if (!confirm(`Are you sure you want to delete this attendance record for ${record.employee_name}?`)) {
      return
    }
          ...
          <Card>
            <div className="mb-4 flex items-center justify-between">
              ...
              <p className="text-sm text-gray-500">
                Showing {attendance.length} record(s) for {new Date(selectedDate).toLocaleDateString()}
              </p>
            </div>
                  ...
                  {attendance.length === 0 ? (
                    ...
                    attendance.map((record) => (
                      <TableRow key={record.id}>
                        ...
                        <TableCell>
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[record.status] || 'bg-gray-100 text-gray-800'}`}>
            ...
            {formData.check_in_time && formData.check_out_time && (
              <div className="p-3 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-700">
                  Hours worked: <strong>{calculateHours(formData.check_in_time, formData.check_out_time)}h</strong>
                </p>
              </div>
            )}
