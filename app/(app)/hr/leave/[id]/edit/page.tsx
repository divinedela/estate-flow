...
export default function EditLeaveRequestPage() {
  ...
  async function fetchData() {
    ...
    const { data: requestData } = await (supabase.from("leave_requests") as any)
      .select(
        `
        *,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (formData.status !== originalStatus) {
      if (formData.status === "approved" || formData.status === "rejected") {
        updateData.approved_by = (appUser as any)?.id;
    ...
    const currentYear = new Date().getFullYear();
    const { data: currentBalanceData } = await (
      supabase.from("leave_balances") as any
    )
      .select("*")
