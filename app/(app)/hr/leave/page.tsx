...
export default function LeavePage() {
  ...
  async function fetchLeaveRequests() {
    ...
    const filteredData = (rawData || [])
      .filter(
        (req) =>
          req.employee?.organization_id === (profile as any).organization_id
      )
      .map((req) => ({
        id: req.id,
      ...
      await (supabase.from("leave_balances") as any)
        .update({
          ...
          pending_days: Math.max(
            0,
            (balance.pending_days || 0) - request.days_requested
          ),
        })
        .eq("id", balance.id);
    }
    ...
    if (rejectBalance) {
      await (supabase.from("leave_balances") as any)
        .update({
          pending_days: Math.max(
            0,
            (rejectBalance.pending_days || 0) - selectedRequest.days_requested
          ),
        })
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">
                  {selectedRequest.days_requested} day(s)
                </p>
              </div>
              <div>
