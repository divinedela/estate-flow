...
async function getHRStats() {
  ...
  const { data: allEmployees } = await supabase
    .from("employees")
    .select(
      "id, first_name, last_name, employee_number, department, position, status, hire_date"
    )
  ...
  if (employeeIds.length > 0) {
    ...
    const { count: validCount } = await supabase
      ...
      .in("employee_id", employeeIds)
      .or(
        `expiry_date.is.null,expiry_date.gt.${
    ...
    documentExpiry.expiringSoon =
      documentExpiry.expiring30Days + documentExpiry.expiring60Days;
  }

  // Today's attendance
  let attendanceToday: AttendanceToday = {
    present: 0,
    absent: 0,
    late: 0,
    onLeave: 0,
  ...
  if (employeeIds.length > 0) {
    ...
    if (todayAttendance) {
      attendanceToday = {
        present: (todayAttendance as any[]).filter(
          (a: any) => a.status === "present"
        ).length,
        absent: (todayAttendance as any[]).filter(
          (a: any) => a.status === "absent"
        ).length,
        late: (todayAttendance as any[]).filter((a: any) => a.status === "late")
          .length,
        onLeave: (todayAttendance as any[]).filter(
          (a: any) => a.status === "on_leave"
        ).length,
      };

      // Get absent employees
      const absentEmployeeIds = (todayAttendance as any[])
  ...
  if (employeeIds.length > 0) {
    ...
    if (leaveBalances) {
      ...
      leaveBalanceSummary = Array.from(balanceMap.entries()).map(
        ([leave_type, data]) => ({
          leave_type,
          ...
          <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-6 text-white">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-4xl font-bold mt-2">
                  {stats.documentExpiry.overdue +
                    stats.documentExpiry.expiringSoon}
                </p>
                <p className="text-red-100 text-xs mt-2">
                  {stats.documentExpiry.overdue} overdue,{" "}
                  {stats.documentExpiry.expiring30Days} expiring soon
                </p>
              </div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm text-gray-600">
                        ...
                        {balance.allocated.toFixed(1)} days
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${
                            balance.allocated > 0
                              ? (balance.remaining / balance.allocated) * 100
                              : 0
                          }%`,
                        }}
                      ></div>
            ...
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-amber-600">
                  {stats.documentExpiry.expiring30Days}
                </span>
              </div>
            ...
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-yellow-600">
                  {stats.documentExpiry.expiring60Days}
                </span>
              </div>
            ...
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-green-600">
                  {stats.documentExpiry.valid}
                </span>
              </div>
              <p className="text-xs font-medium text-green-700">Valid</p>
            </div>
          </div>
          {(stats.documentExpiry.overdue > 0 ||
            stats.documentExpiry.expiring30Days > 0) && (
            <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
              <div className="flex items-center gap-2">
                <ExclamationTriangleIcon className="h-5 w-5 text-red-600" />
                <p className="text-sm font-medium text-red-700">
                  {stats.documentExpiry.overdue} document(s) need immediate
                  renewal, {stats.documentExpiry.expiring30Days} expiring within
                  30 days
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
              ...
              <p className="text-xs text-indigo-600 mt-1">
                Note: Recruitment module coming soon
              </p>
            </div>
            ...
            <Link href="/hr/documents">
              <div className="p-4 bg-white border-2 border-orange-200 rounded-lg hover:border-orange-400 hover:shadow-md transition-all cursor-pointer group">
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-sm font-semibold text-gray-900 group-hover:text-orange-600">
                    Manage Documents
                  </p>
                </div>
              </div>
            ...
            <Link href="/hr/attendance">
              <div className="p-4 bg-white border-2 border-purple-200 rounded-lg hover:border-purple-400 hover:shadow-md transition-all cursor-pointer group">
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-sm font-semibold text-gray-900 group-hover:text-purple-600">
                    View Attendance
                  </p>
                </div>
              </div>
