...
export default function DocumentsPage() {
  ...
  async function fetchDocuments() {
    ...
    let query = (supabase.from("employee_documents") as any)
      .select(
        `
        *,
    ...
    const filteredData = (rawData || [])
      .filter(
        (d) => d.employee?.organization_id === (profile as any).organization_id
      )
      .map((d) => ({
        id: d.id,
  ...
  async function uploadFile(
    ...
    try {
      ...
      const fileName = `${employeeId}/${Date.now()}-${Math.random()
        .toString(36)
        .substring(7)}.${fileExt}`;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingDocument) {
      ...
      const { error } = await (
        supabase.from("employee_documents") as any
      ).insert(documentData);
  ...
  const stats = {
    ...
    expiringSoon: documents.filter((d) => {
      ...
      const daysUntil = Math.ceil(
        (new Date(d.expiry_date).getTime() - new Date().getTime()) /
          (1000 * 60 * 60 * 24)
      );
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Document File{" "}
              {!editingDocument && <span className="text-red-500">*</span>}
            </label>
                ...
                {selectedFile ? (
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-sm text-gray-600">
                      {editingDocument.file_name || "Current file"}
                    </p>
                    <p className="text-xs text-gray-500">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {viewingDocument.employee_number}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <span
                  className={`px-2 py-1 text-xs font-medium rounded-full ${
                    getExpiryStatus(viewingDocument.expiry_date).color
                  }`}
                >
