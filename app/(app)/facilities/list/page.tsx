...
export default function FacilitiesListPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingFacility) {
      ...
      await (supabase.from("facilities") as any).insert({
        ...facilityData,
  ...
  const stats = {
    ...
    underConstruction: facilities.filter(
      (f) => f.status === "under_construction"
    ).length,
              ...
              <TableBody>
                {filteredFacilities.map((facility) => (
                  <TableRow key={facility.id}>
                    ...
                    <TableCell>
                      {facility.total_area
                        ? `${facility.total_area.toLocaleString()} sqft`
                        : "-"}
                    </TableCell>
                    <TableCell>
                      <span
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">
                  {viewingFacility.facility_type?.replace("_", " ") || "N/A"}
                </p>
              </div>
              <div>
                ...
                <span
                  className={`px-2 py-1 text-xs font-medium rounded-full ${
                    statusColors[viewingFacility.status]
                  }`}
                >
            ...
            <div>
              ...
              <p className="text-gray-600">
                {[
                  viewingFacility.city,
                  viewingFacility.state,
                  viewingFacility.postal_code,
                  viewingFacility.country,
                ]
                  .filter(Boolean)
                  .join(", ")}
              </p>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-gray-50 p-4 rounded-lg text-center">
                <p className="text-2xl font-bold text-blue-600">
                  {viewingFacility.total_units || 0}
                </p>
                <p className="text-sm text-gray-500">Units</p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg text-center">
                <p className="text-2xl font-bold text-green-600">
                  {viewingFacility.total_area?.toLocaleString() || 0}
                </p>
                <p className="text-sm text-gray-500">Total Area (sqft)</p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg text-center">
                <p className="text-2xl font-bold text-purple-600">
                  {viewingFacility.year_built || "N/A"}
                </p>
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-600">
            ...
            undone and will also remove all associated units and maintenance
            records.
          </p>
