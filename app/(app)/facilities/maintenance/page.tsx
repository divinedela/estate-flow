...
export default function MaintenancePage() {
  ...
  async function fetchData() {
    ...
    const [requestsRes, facilitiesRes, unitsRes, assetsRes] = await Promise.all([
      supabase
        ...
        .select('*, facility:facilities(name), unit:facility_units(unit_number), asset:assets(name)')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingRequest) {
      ...
      if (formData.status === 'completed' && editingRequest.status !== 'completed') {
          ...
          <Card>
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="mt-2 text-gray-500">Create a new request to get started.</p>
                    ...
                    <TableRow key={request.id}>
                      ...
                      <TableCell>
                        <select
                          ...
                          className={`text-xs font-medium rounded-full px-2 py-1 border-0 ${statusColors[request.status]}`}
                        >
                          <option value="open">Open</option>
                          <option value="assigned">Assigned</option>
                          <option value="in_progress">In Progress</option>
        ...
        >
          {viewingRequest && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-semibold text-gray-900">{viewingRequest.title}</h3>
                  <p className="text-gray-500">{viewingRequest.request_number}</p>
                </div>
                <span className={`px-3 py-1 text-sm font-medium rounded-full ${statusColors[viewingRequest.status]}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{(viewingRequest.facility as { name: string } | undefined)?.name || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Unit</p>
                  <p className="font-medium">{(viewingRequest.unit as { unit_number: string } | undefined)?.unit_number || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Type</p>
                  <p className="font-medium">{viewingRequest.request_type || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Priority</p>
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${priorityColors[viewingRequest.priority]}`}>
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {viewingRequest.requested_date 
                      ? new Date(viewingRequest.requested_date).toLocaleDateString() 
                      : 'N/A'}
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-medium">
                    {viewingRequest.completed_date 
                      ? new Date(viewingRequest.completed_date).toLocaleDateString() 
                      : 'Not completed'}
                  </p>
                </div>
              </div>
              ...
              {viewingRequest.completion_notes && (
                <div>
                  ...
                  <p className="text-gray-700">{viewingRequest.completion_notes}</p>
                </div>
              )}
