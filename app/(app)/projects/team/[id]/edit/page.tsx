...
export default function EditTeamMemberPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: userError } = await (supabase.from("app_users") as any)
        .update({
      ...
      if (currentRoles && currentRoles.length > 0) {
        const currentRole = (currentRoles[0] as any).role;
        if (currentRole.name !== formData.role) {
          ...
          if (newRole) {
            ...
            const { error: roleError } = await (
              ...
              .update({ role_id: (newRole as any).id })
              .eq("user_id", memberId);
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <Card className="max-w-md w-full">
            ...
            <p className="text-sm text-gray-500 mb-4">
              ...
              cannot be undone. All project assignments will be removed.
            </p>
