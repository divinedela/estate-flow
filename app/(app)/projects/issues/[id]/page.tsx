...
async function getIssueDetails(id: string): Promise<IssueDetails | null> {
  ...
  return {
    ...
    project_name: issueData.project?.name || 'Unknown',
    project_code: issueData.project?.project_code || 'N/A',
    reported_by: issueData.reporter?.full_name || 'Unknown',
    assigned_to: issueData.assignee?.full_name || null,
    created_at: issueData.created_at,
    resolved_at: issueData.resolved_date,
  }
}
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          <Card>
            <div className="flex items-center">
              <div className={`p-3 rounded-full ${issue.status === 'resolved' || issue.status === 'closed' ? 'bg-green-100' : 'bg-yellow-100'}`}>
          ...
          <Card>
            <div className="flex items-center">
              <div className={`p-3 rounded-full ${issue.severity === 'critical' ? 'bg-red-100' : issue.severity === 'high' ? 'bg-orange-100' : 'bg-yellow-100'}`}>
                <SeverityIcon className={`h-6 w-6 ${issue.severity === 'critical' ? 'text-red-600' : issue.severity === 'high' ? 'text-orange-600' : 'text-yellow-600'}`} />
              </div>
              <div className="ml-4">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${typeColors[issue.issue_type] || typeColors.other}`}>
                      {issue.issue_type}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">{issue.project_name}</p>
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <span className={`mt-1 inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${severityColors[issue.severity] || severityColors.medium}`}>
                    {issue.severity}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <span className={`mt-1 inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${statusColors[issue.status] || statusColors.open}`}>
                    {issue.status.replace('_', ' ')}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <span className={`mt-1 inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${typeColors[issue.issue_type] || typeColors.other}`}>
                    {issue.issue_type}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-gray-900">{issue.reported_by}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <UserIcon className="h-4 w-4 text-gray-400 mr-2" />
                    <span className="text-sm text-gray-900">{issue.assigned_to || 'Unassigned'}</span>
                  </div>
                </div>
                ...
                {issue.resolved_at && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 uppercase mb-1">Resolved Date</p>
                    <div className="flex items-center">
            ...
            <Card>
              ...
              <div className="space-y-3">
                <Link href={`/projects/issues/${issue.id}/edit`} className="block">
