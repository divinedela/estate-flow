...
export default function PhasesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this phase? This will also affect any tasks linked to this phase.')) return
                ...
                <TableBody>
                  {filteredPhases.length === 0 ? (
                    ...
                    filteredPhases.map((phase) => {
                      ...
                      const isLast = projectPhases[projectPhases.length - 1]?.id === phase.id
                        ...
                        <TableRow key={phase.id}>
                            ...
                            <div className="flex items-center space-x-1">
                              ...
                              <div className="flex flex-col">
                                <button
                                  ...
                                  disabled={isFirst}
                                  className={`p-0.5 ${isFirst ? 'text-gray-300' : 'text-gray-500 hover:text-indigo-600'}`}
                                >
                                ...
                                <button
                                  ...
                                  disabled={isLast}
                                  className={`p-0.5 ${isLast ? 'text-gray-300' : 'text-gray-500 hover:text-indigo-600'}`}
                                >
                          ...
                          <TableCell>
                            <div>
                              ...
                              {phase.description && (
                                <p className="text-xs text-gray-500 truncate max-w-xs">{phase.description}</p>
                              )}
                            </div>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[phase.status] || 'bg-gray-100 text-gray-800'}`}>
                          ...
                          <TableCell>
                            <div className="text-xs text-gray-600">
                              {phase.start_date ? (
                                <>
                                  <p>Start: {new Date(phase.start_date).toLocaleDateString()}</p>
                                  {phase.end_date && <p>End: {new Date(phase.end_date).toLocaleDateString()}</p>}
                                </>
                              ) : (
                                'No dates set'
                              )}
                          ...
                          <TableCell>
                            <div className="text-sm">
                              <p className="text-gray-600">{formatCurrency(phase.budget)}</p>
                              {phase.actual_cost != null && phase.actual_cost > 0 && (
                                <p className="text-xs text-gray-500">Actual: {formatCurrency(phase.actual_cost)}</p>
                              )}
                            </div>
                          </TableCell>
                          <TableCell>
