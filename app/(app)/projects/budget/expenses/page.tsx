...
export default async function ExpensesListPage() {
  ...
  const paidExpenses = expenses.filter(e => e.payment_status === 'paid').reduce((sum, exp) => sum + exp.amount, 0)
  const pendingExpenses = expenses.filter(e => e.payment_status === 'pending').reduce((sum, exp) => sum + exp.amount, 0)
          ...
          <Card>
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">{formatCurrency(paidExpenses)}</p>
              </div>
            </div>
          </Card>
              ...
              <TableBody>
                {expenses.length === 0 ? (
                  ...
                  expenses.map((expense) => {
                    const StatusIcon = statusIcons[expense.payment_status] || ClockIcon
                    return (
                      <TableRow key={expense.id} className="hover:bg-gray-50">
                        ...
                        <TableCell>
                          <div className="flex items-center">
                            ...
                            <div>
                              <p className="text-sm font-medium text-gray-900">{expense.project_code}</p>
                              <p className="text-xs text-gray-500">{expense.project_name}</p>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <p className="text-sm text-gray-900 max-w-xs truncate">{expense.description}</p>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${statusColors[expense.payment_status] || statusColors.pending}`}>
