...
async function getProjectBudgets() {
  ...
  const { data: allProjects } = await supabase
    .from("projects")
    .select(
      "id, project_code, name, status, budget, actual_cost, progress_percentage"
    )
  ...
  const onBudgetCount = projects.filter(
    (p: any) => (p.variance || 0) >= 0
  ).length;
            ...
            <div className="space-y-4">
              {stats.projects.slice(0, 5).map((project: any) => {
                const budgetPercentage =
                  stats.totalBudget > 0
                    ? ((project.budget || 0) / stats.totalBudget) * 100
                    : 0;
                const actualPercentage =
                  stats.totalBudget > 0
                    ? ((project.actual_cost || 0) / stats.totalBudget) * 100
                    : 0;
                return (
                  <div key={project.id}>
                    ...
                    <TableRow key={project.id} className="hover:bg-gray-50">
                      ...
                      <TableCell>
                        <div className="flex items-center">
                          ...
                          >
                            {formatCurrency(Math.abs(project.variance || 0))}
                          </span>
                        </div>
                        <p className="text-xs text-gray-500">
                          {(project.variance_percentage || 0) >= 0 ? "+" : ""}
                          {(project.variance_percentage || 0).toFixed(1)}%
                        </p>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center">
