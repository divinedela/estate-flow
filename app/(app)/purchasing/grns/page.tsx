...
import { ClipboardDocumentCheckIcon, PlusIcon, EyeIcon, ArrowLeftIcon } from '@heroicons/react/24/outline'
...
export default function GoodsReceiptsPage() {
  ...
  async function fetchData() {
    ...
    setReceipts(grnsData?.map(grn => ({
      ...grn,
      po_number: grn.po?.po_number,
      supplier_name: grn.po?.supplier?.name
    })) || [])
    
    setPurchaseOrders(posData?.map(po => ({
      ...po,
  ...
  return (
    <RoleGuard allowedRoles={['super_admin', 'procurement_officer', 'inventory_officer']}>
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Goods Receipts (GRN)</h1>
                  ...
                  {filteredReceipts.length === 0 ? (
                    ...
                    filteredReceipts.map((grn) => (
                      <TableRow key={grn.id}>
                        ...
                        <TableCell>
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[grn.status]}`}>
                            {grn.status}
                          </span>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            ...
                            {grn.status === 'pending' && (
                              <select
                                value={grn.status}
                                onChange={(e) => updateStatus(grn.id, e.target.value)}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-800">
                After creating the GRN, you can add individual line items and quantities received.
              </p>
            </div>
