...
export default function PurchaseRequisitionsPage() {
  ...
  const [rejectingPR, setRejectingPR] = useState<PurchaseRequisition | null>(
    null
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingPR) {
      await (supabase.from("purchase_requisitions") as any)
        .update(prData as any)
        .eq("id", editingPR.id);
    } else {
      const prNumber = `PR-${Date.now().toString().slice(-8)}`;
      await (supabase.from("purchase_requisitions") as any).insert({
        ...prData,
        pr_number: prNumber,
  ...
  async function convertToPO(pr: PurchaseRequisition) {
    if (confirm("Convert this PR to a Purchase Order?")) {
      ...
      alert(
        "PR marked as converted. Please create the PO in the Purchase Orders section."
      );
    }
  }
  ...
  return (
    <RoleGuard allowedRoles={["super_admin", "procurement_officer"]}>
      ...
      <Modal
        ...
        title={
          editingPR ? "Edit Purchase Requisition" : "New Purchase Requisition"
        }
        size="lg"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              After creating the PR, you can submit it for approval. Once
              approved, it can be converted to a Purchase Order.
            </p>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Requested by {viewingPR.requester_name || "Unknown"} on{" "}
                  {new Date(viewingPR.created_at).toLocaleDateString()}
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <span
                  className={`px-3 py-1 text-sm font-medium rounded-full ${
                    priorityColors[viewingPR.priority]
                  }`}
                >
                  {viewingPR.priority}
                </span>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {viewingPR.total_amount
                        ? `$${viewingPR.total_amount.toLocaleString()}`
                        : "N/A"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span>
                      {viewingPR.required_date
                        ? new Date(viewingPR.required_date).toLocaleDateString()
                        : "N/A"}
                    </span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span>
                        {new Date(viewingPR.approved_at).toLocaleDateString()}
                      </span>
                    </div>
                  </div>
                </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Are you sure you want to reject PR{" "}
            <strong>{rejectingPR?.pr_number}</strong>?
          </p>
          <FormTextarea
