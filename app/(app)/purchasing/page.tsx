...
export default async function PurchasingDashboardPage() {
  ...
  const { data: prsData } = await supabase
    ...
    .select(
      "id, pr_number, status, priority, total_amount, required_date, created_at, requester:app_users!requested_by(full_name)"
    )
    .eq("organization_id", organizationId)
  ...
  const { data: posData } = await supabase
    ...
    .select(
      "id, po_number, status, total_amount, issued_date, expected_delivery_date, supplier:suppliers(name)"
    )
    .eq("organization_id", organizationId)
    .order("created_at", { ascending: false })
    .limit(5);
  ...
  const activeSuppliers =
    (suppliersData as any[])?.filter((s: any) => s.status === "active")
      .length || 0;

  const totalPRs = allPrsData?.length || 0;
  const pendingPRs =
    (allPrsData as any[])?.filter((pr: any) => pr.status === "submitted")
      .length || 0;
  const approvedPRs =
    (allPrsData as any[])?.filter((pr: any) => pr.status === "approved")
      .length || 0;
  const prTotalValue =
    (allPrsData as any[])?.reduce(
  ...
  const openPOs =
    (allPosData as any[])?.filter((po: any) =>
      ["sent", "confirmed", "partial"].includes(po.status)
    ).length || 0;
  const receivedPOs =
    (allPosData as any[])?.filter((po: any) => po.status === "received")
      .length || 0;
  const poTotalValue =
    (allPosData as any[])?.reduce(
  ...
  const pendingGRNs =
    (grnsData as any[])?.filter((g: any) => g.status === "pending").length || 0;

  const totalInvoices = invoicesData?.length || 0;
  const pendingInvoices =
    (invoicesData as any[])?.filter((i: any) => i.status === "pending")
      .length || 0;
  const today = new Date();
  const overdueInvoices =
    (invoicesData as any[])?.filter((i: any) => {
  ...
  const totalOutstanding =
    (invoicesData as any[])
      ...
      .reduce(
        (sum: number, i: any) => sum + (i.total_amount - (i.amount_paid || 0)),
        0
      ) || 0;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-emerald-100">
                Manage procurement, suppliers, and vendor relationships
              </p>
            </div>
                      ...
                      <tr key={pr.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 whitespace-nowrap">
                          ...
                          <p className="text-xs text-gray-500">
                            {pr.requester?.full_name || "N/A"}
                          </p>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span
                            className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              priorityColors[pr.priority]
                            }`}
                          >
                            {pr.priority}
                          </span>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="font-medium">
                    {totalPRs > 0
                      ? Math.round((approvedPRs / totalPRs) * 100)
                      : 0}
                    %
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${
                        totalPRs > 0 ? (approvedPRs / totalPRs) * 100 : 0
                      }%`,
                    }}
                  ></div>
                </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="font-medium">
                    {totalPOs > 0
                      ? Math.round((receivedPOs / totalPOs) * 100)
                      : 0}
                    %
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${
                        totalPOs > 0 ? (receivedPOs / totalPOs) * 100 : 0
                      }%`,
                    }}
                  ></div>
                </div>
