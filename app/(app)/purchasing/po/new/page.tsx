...
export default function NewPOPage() {
      ...
      if (prItems && prItems.length > 0) {
        setPOItems(
          prItems.map((item: any, index) => ({
            ...
            quantity: (item as any).quantity.toString(),
            unit_of_measure: (item as any).unit_of_measure || "pcs",
            unit_price: (item as any).estimated_unit_price
              ? (item as any).estimated_unit_price.toString()
              : "",
            total_price: (item as any).estimated_total_price
              ? (item as any).estimated_total_price.toString()
              : "",
  ...
  const removeItem = (id: string) => {
    if (poItems.length > 1) {
      setPOItems(poItems.filter((item: any) => (item as any).id !== id));
    }
  };
    ...
    try {
      ...
      const validItems = poItems.filter(
        (item: any) =>
          ...
          parseFloat((item as any).unit_price) > 0
      );

      if (validItems.length === 0) {
        alert(
          "Please add at least one item with description, quantity, and unit price"
        );
        setLoading(false);
        return;
      }
      ...
      const itemsData = validItems.map((item: any) => ({
        ...
        unit_price: parseFloat((item as any).unit_price),
        total_price:
          parseFloat((item as any).total_price) ||
          parseFloat((item as any).quantity) *
            parseFloat((item as any).unit_price),
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <FormSelect
                ...
                onChange={(e) =>
                  setFormData({ ...formData, pr_id: e.target.value })
                }
                options={[
                  { value: "", label: "No PR" },
                  ...prs.map((pr) => ({
                    value: pr.id,
                    label: `${pr.pr_number} - ${
                      pr.total_amount ? `$${pr.total_amount}` : "N/A"
                    }`,
                  })),
                ]}
              />
